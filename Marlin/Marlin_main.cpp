...
bool enqueue_and_echo_command(const char* cmd) {
  ...
  void enqueue_and_echo_command_now(const char* cmd) {
   ...
   void setup_autopoweroffhi()
   {
	   digitalWrite(POWEROFFPIN,HIGH);
   }
   
   void setup_autopowerofflo()
   {
		  ...
		  if (inp0.State() == PS_START)
		  {
			  ...
			  {	
					  ...
					  if (IsStopped()) {
						  ...
						  if (gpos) {
							  ...
							  case 5:
								  ...
								  dgus.ShowErrorMessage(MMSG_STOPPED);
								  break;
							  }
						  }
					  }
				  ...
				  {   // Handle escapes
					  ...
					  if (!serial_comment_mode0) serial_line_buffer0[serial_count0++] = serial_char;		// 将字符存入行缓冲区
				  }
...
void clean_up_after_endstop_or_probe_move() {
  ...
  float probe_pt(const float &rx, const float &ry, const ProbePtRaise raise_after/*=PROBE_PT_NONE*/, const uint8_t verbose_level/*=0*/, const bool probe_relative/*=true*/) {
    ...
    if (isnan(measured_z)) 
	{
		dgus.ShowErrorMessage(MMSG_ERR_PROBING_FAILED[wtvar_language]);
      SERIAL_ERROR_START();
...
inline void gcode_G4() {
  ...
  inline void home_z_safely() {
    ...
    if (!TEST(axis_known_position, X_AXIS) || !TEST(axis_known_position, Y_AXIS)) {
      dgus.ShowErrorMessage(MMSG_ERR_Z_HOMING);
      SERIAL_ECHO_START();
    ...
    if (position_is_reachable(destination[X_AXIS], destination[Y_AXIS])) {
      ...
      dgus.ShowErrorMessage(MMSG_ZPROBE_OUT);
      SERIAL_ECHO_START();
...
inline void gcode_M109() {
  ...
  inline void gcode_M190() {
	...
	dgus.ShowMessage(thermalManager.isHeatingBed() ? PSTR(MSG_BED_HEATING) : PSTR(MSG_BED_COOLING));
...
{	
		int ret = extractFront((const char*)parser.string_arg, "Progress=", parsedString);
		if (ret)
		{
			progress_bar_percent = atoi(parsedString);
		}
		else
			dgus.ShowMessage(parser.string_arg);
	}
  ...
  inline void gcode_M428() {
    ...
    LOOP_XYZ(i) {
      ...
      if (!WITHIN(diff[i], -20, 20)) {
		...
		dgus.ShowErrorMessage("Err: Too far!");
        BUZZ(200, 40);
        return;
      }
    }
    ...
    report_current_position();
	dgus.ShowErrorMessage(MSG_HOME_OFFSETS_APPLIED);
    BUZZ(100, 659);
    BUZZ(100, 698);
	*/
  }
...
void setup() {
  ...
  #ifdef AUTOPOWEROFF
    setup_autopoweroffpin();
  #endif
  
  dgus.Init(DGUS_BAUDRATE);
  ...
  strcpy_P(esp32_name, MMSG_UNKNOWN);
	strcpy_P(esp32_serial, MMSG_UNKNOWN);
	strcpy_P(esp32_version, MMSG_UNKNOWN);
	strcpy_P(esp32_hardware, MMSG_UNKNOWN);
	strcpy_P(tb_state, MMSG_UNKNOWN);
	...
	if (wtvar_gohome == 1)
	{
		...
		wtvar_skipTest = 1;
		(void)settings.save();
		enqueue_and_echo_commands_P(PSTR("G28 X Y F3000\r\nG1 Y270\r\nM18"));
    #if ENABLED(POWER_LOSS_RECOVERY)
        card.removeJobRecoveryFile();
    #endif
	}
