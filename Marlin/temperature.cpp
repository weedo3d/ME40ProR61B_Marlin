...
void Temperature::max_temp_error(const int8_t e) {
  _temp_error(e, PSTR(MSG_T_MAXTEMP), MMSG_ERROR_MAXTEMP[wtvar_language]);  
}
...
void Temperature::min_temp_error(const int8_t e) {
	if (e == -1)
	{
		if (dgus.isTesting())
			...
			_temp_error(e, PSTR(MSG_T_MINTEMP), MMSG_ERROR_BEDTEMP[wtvar_language]);
	}
	else
	{
		if (dgus.isTesting())
			...
			_temp_error(e, PSTR(MSG_T_MINTEMP), MMSG_ERROR_MINTEMP[wtvar_language]);
	}
}
