...
void Packet::Reset()
{
	...
	memset((char *)payload, 0 , BUFFER_SIZE);
	rcode = RC_OK;
	state = PS_START;
}
...
OutPacket::OutPacket (void)
{
	BUFFER_SIZE = OUT_PACKET_PAYLOAD;
	payload = (uint8_t*)malloc(OUT_PACKET_PAYLOAD); 
}

InPacket::InPacket (void)
{
	BUFFER_SIZE = MAX_PACKET_PAYLOAD;
	payload = (uint8_t*)malloc(MAX_PACKET_PAYLOAD); 
}
